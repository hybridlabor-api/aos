...
case "$OPENWIKI_PROVIDER" in
    ...
    grok|xai)   API_KEY_VAR="XAI_API_KEY" ;;
    nvidia)     API_KEY_VAR="NVIDIA_API_KEY" ;;
    openrouter) API_KEY_VAR="OPENROUTER_API_KEY" ;;
    openai)     API_KEY_VAR="OPENAI_API_KEY" ;;
    custom)     API_KEY_VAR="OPENWIKI_API_KEY" ;;
    ollama)     API_KEY_VAR="" ;;
    *)          OPENWIKI_PROVIDER="google"; API_KEY_VAR="GEMINI_API_KEY" ;;
esac

# For Google provider, use the existing interactive flow with verification
if [ "$OPENWIKI_PROVIDER" = "google" ]; then
    ...
    if [ -z "$API_KEY" ]; then
        echo "No API key provided for provider '$OPENWIKI_PROVIDER'. Daemon will run in collect-only mode."
    fi
fi
...
write_launcher() {
    ...
    (
        umask 077
        {
            ...
            printf '# Holds the OpenWiki provider config and API keys so they stay out of\n'
            printf '# the LaunchAgent plist and the systemd unit. Mode 700 on purpose.\n'
            printf 'HOME=%s; export HOME\n' "$(sq "$HOME")"
            printf 'PATH=%s; export PATH\n' "$(sq "$path_value")"
            if [ -n "$OPENWIKI_PROVIDER" ]; then
                printf 'OPENWIKI_PROVIDER=%s; export OPENWIKI_PROVIDER\n' "$(sq "$OPENWIKI_PROVIDER")"
            fi
            if [ -n "$OPENWIKI_MODEL" ]; then
                printf 'OPENWIKI_MODEL=%s; export OPENWIKI_MODEL\n' "$(sq "$OPENWIKI_MODEL")"
            fi
            if [ -n "$OPENWIKI_BASE_URL" ]; then
                printf 'OPENWIKI_BASE_URL=%s; export OPENWIKI_BASE_URL\n' "$(sq "$OPENWIKI_BASE_URL")"
            fi
            if [ -n "$API_KEY" ]; then
                case "$OPENWIKI_PROVIDER" in
                    google)     printf 'GEMINI_API_KEY=%s; export GEMINI_API_KEY\n' "$(sq "$API_KEY")" ;;
                    groq)       printf 'GROQ_API_KEY=%s; export GROQ_API_KEY\n' "$(sq "$API_KEY")" ;;
                    grok|xai)   printf 'XAI_API_KEY=%s; export XAI_API_KEY\n' "$(sq "$API_KEY")" ;;
                    nvidia)     printf 'NVIDIA_API_KEY=%s; export NVIDIA_API_KEY\n' "$(sq "$API_KEY")" ;;
                    openrouter) printf 'OPENROUTER_API_KEY=%s; export OPENROUTER_API_KEY\n' "$(sq "$API_KEY")" ;;
                    openai)     printf 'OPENAI_API_KEY=%s; export OPENAI_API_KEY\n' "$(sq "$API_KEY")" ;;
                    custom)     printf 'OPENWIKI_API_KEY=%s; export OPENWIKI_API_KEY\n' "$(sq "$API_KEY")" ;;
                esac
            fi
...
if [ "$OS" = "macos" ]; then
    ...
    echo " -> Launcher written to $LAUNCHER_PATH (mode 700; holds the provider config and API key)."
