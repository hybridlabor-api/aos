...
Do not use this skill for validating an existing script only. Use `bash-script-validator` for validation-only requests.
...
| Validation | `bash-script-validator` | Local checks: `bash -n`, `shellcheck` if available, `sh -n` for POSIX mode |
...
1. Invoke `bash-script-validator`
2. Apply fixes
3. Re-run validation
...
6. Validate with `bash-script-validator` and rerun until clean.
