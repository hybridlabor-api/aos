...
Generate production-ready Makefiles with best practices for C/C++, Python, Go, Java, and generic projects. Features GNU Coding Standards compliance, standard targets, security hardening, and automatic validation via makefile-validator skill.
...
**Do NOT use for:** Validating existing Makefiles (use makefile-validator), debugging (use `make -d`), or running builds.
...
1. **Preferred path:** run `makefile-validator`.
