 ...
 */
async function renderKineticIntro({ rotations = 1, fps = 12.5, skip = false, pkgVersion = '4.0.0' } = {}) {
    ...
    process.stdout.write(`\x1b]0;BDB Agent OS · AOS Installer v${pkgVersion}\x07`);
    ...
    return new Promise((resolve) => {
        ...
        const timer = setInterval(() => {
            ...
            const buffer = [
                ...
                '\x1b[2K',
                `\x1b[2K  ${BRAND.lila}${BRAND.bold}━━━ BDB AGENT OS ━━━${BRAND.reset}  ${BRAND.white}${BRAND.bold}AOS Kernel v${pkgVersion}${BRAND.reset}  ${BRAND.dim}· Initializing runtime...${BRAND.reset}`
            ].join('\n');
...
function buildTelemetryCard({ installState, detections, daemonStatus = [] }) {
    ...
    const SHORT_NAMES = {
        ...
        // The underlying detection (installer.js's 'vscode' evidence) fires on
        // any of: the VS Code app/CLI being present, OR a Roo/Cline extension
        // glob match. Those are not the same claim -- plain VS Code with no
        // Roo/Cline extension installed used to display as "Roo / Cline" here,
        // asserting something that was not there. "VS Code" is the one label
        // that is true in all three trigger cases.
        'Roo Code / Cline / VS Code': 'VS Code',
        'Aider CLI': 'Aider',
